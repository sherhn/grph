...
class App(tk.Tk):
    ...
    def _build_sidebar(self, parent: ttk.Frame):
        ...
        ttk.Button(preset_frame1, text="Прямоугольный", command=self._set_right_triangle, width=14).pack(side=tk.LEFT, padx=1)
        ttk.Button(preset_frame1, text="Равносторонний", command=self._set_equilateral_triangle, width=14).pack(side=tk.LEFT, padx=1)

        preset_frame2 = ttk.Frame(tri_lf)
        preset_frame2.pack(fill=tk.X, pady=2)
        ttk.Button(preset_frame2, text="Тупоугольный", command=self._set_obtuse_triangle, width=14).pack(side=tk.LEFT, padx=1)
        ttk.Button(preset_frame2, text="Случайный", command=self._set_random_triangle, width=14).pack(side=tk.LEFT, padx=1)

        ttk.Button(tri_lf, text="📂 Импорт из SVG...", command=self._import_svg_dialog).pack(fill=tk.X, pady=(6, 2))
        ...
        ttk.Button(parent, text="💾 Сохранить рисунок", command=self._save_image_dialog).pack(fill=tk.X, pady=(10, 2), padx=4)
    ...
    def _import_svg_dialog(self):
        ...
        if filename:
            ...
            if pts:
                self.var_ax.set(pts[0][0])
                self.var_ay.set(pts[0][1])
                self.var_bx.set(pts[1][0])
                self.var_by.set(pts[1][1])
                self.var_cx.set(pts[2][0])
                self.var_cy.set(pts[2][1])
                self._update_and_render()
            else:
                messagebox.showwarning("Ошибка импорта", "В файле не найден треугольник (<polygon>, <path> или <polyline>).")
